...
response=urllib.request.urlopen('https://tw.movies.yahoo.com/movieinfo_main.html/id=6098')
...
class myparser(HTMLParser):
    ...
    def handle_data(self, data):
        if self.isNumber ==1:
            ...
            if (224<=self.getpos()[0]<=224) and self.tag == 'a':
                self.actor.append(data)
            if (225<=self.getpos()[0]<=225) and self.attr == [('class', 'dta')]:
                self.company.append(data)
            if (226<=self.getpos()[0]<=226) and self.tag == 'a':
                self.website.append(data)
            if (449<=self.getpos()[0]<=449) and self.attr == [('class', 'text full')]:
...
print('電影名稱:',Parser.movietitle_c[0],'\n       ',Parser.movietitle_e[0],'\n上映日期:',Parser.date[0],'\n類型:','、'.join(Parser.type),'\n電影長度:',Parser.length[0],'\n導演:','、'.join(Parser.director),'\n演員:','、'.join(Parser.actor),'\n電影公司:',Parser.company[0],'\n網頁:',Parser.website[0],'\n劇情介紹:\n','、'.join(Parser.plot),file=open('6098.txt','w',encoding='big5'))
